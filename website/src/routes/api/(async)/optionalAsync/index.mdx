...
When using `optionalAsync` in a <Link href="../../pipeAsync/">`pipeAsync`</Link>, the pipe actions only execute if a `default_` value is provided or the key is present. This applies to all pipe actions including <Link href="../../transformAsync/">`transformAsync`</Link>, <Link href="../../checkAsync/">`checkAsync`</Link>, and others.
